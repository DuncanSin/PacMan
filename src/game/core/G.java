...
//			Arrays.fill(edibleTimes,(int)(G.EDIBLE_TIME*(Math.pow(G.EDIBLE_TIME_REDUCTION,totLevel))));						
	...
	{
		return Math.abs(mazes[curMaze].graph[from].x-mazes[curMaze].graph[to].x)+Math.abs(mazes[curMaze].graph[from].y-mazes[curMaze].graph[to].y);
	}
	...
	protected final class Maze
	{
		...
		private void loadNodes(String fileName)
		{
	        ...
	        {         	
	        	//APPLET
	        	BufferedReader br=new BufferedReader(new InputStreamReader(this.getClass().getResourceAsStream(pathMazes+fileName)));
	        	//APPLICATION
